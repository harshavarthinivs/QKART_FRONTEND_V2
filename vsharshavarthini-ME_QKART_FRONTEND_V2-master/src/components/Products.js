...
const Products = () => {
  ...
  const performSearch = async (text) => {
    ...
    let data=await axios.get(config.endpoint+"/products/search?value="+text);
  ...
  const debounceSearch = (event, debounceTimeout) => {
    ...
    // };
    // //  clearTimeout(debounceTimer);
    //  debounceTimer= setTimeout(later,debounceTimeout);
    clearTimeout(debounceTimer);
   ...
   */
    const fetchCart = async (token) => {
      ...
      if(!token) 
          return ;
      let data= await axios.get(config.endpoint+"/cart",{headers:{Authorization: `Bearer ${token}`}});
      // console.log(data);
      setCartData(data.data);
      console.log("cartdata",cartData);
      // setProductsData(data.data);
      // console.log(productsData);
      // {generateCartItemsFrom(cartData, productsData)}
...
  const isItemInCart = (items, productId) => {
  // console.log("isitemincart is called");
  // console.log("iscart",items);
  for(let i of items)
  {
...
const addToCart = async (
  ...
  // console.log("addtocart func is called");
  if(token)
  {
  ...
  return (
    <div>
      ...
      <Grid container>
        ...
        {
          userName ?(<Grid item xs={12} md={3} sx={{bgcolor:"#E9F5E1"}}>
